...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  conn.sendButtonImg(m.chat, await (await fetch(json.result)).buffer(), 'Nih', '© mikebot', 'Next', `${usedPrefix + command} ${text}`, m, 0, { thumbnail: await (await fetch(json.result)).buffer() })
}
